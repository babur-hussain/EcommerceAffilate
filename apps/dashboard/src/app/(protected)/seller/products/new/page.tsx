...
const Section = ({ title, name, children, isActive, onToggle, sectionRef }: SectionProps) => {
  return (
    <div 
      ref={sectionRef}
      className={`border rounded-xl mb-6 transition-all duration-300 ${isActive ? 'border-primary-500 shadow-lg' : 'border-gray-200 shadow-sm hover:border-gray-300'}`}
    >
      <button
        type="button"
      ...
      <div className={`overflow-hidden transition-all duration-300 ${
        ...
        <div className="p-6 bg-white space-y-6 rounded-b-xl">
          {children}
        </div>
      </div>
    </div>
...
export default function NewProductPage() {
  ...
  const uploadImagesToCloudinary = async () => {
    ...
    try {
      ...
      const response = await apiClient.post<{ success: boolean; images: string[]; count: number }>('/api/upload/images', formData, {
        headers: {
  ...
  const handleSaveDraft = async () => {
    ...
    if (!formData.title || !formData.category || !formData.price) {
      toast.error('Please fill in at least Title, Category, and Price to save draft');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (!formData.qualityCheckConfirmed || !formData.authenticityConfirmed || !formData.brandAuthorizationConfirmed) {
    ...
    try {
      ...
      const productData = {
        ...
        category: formData.category,
        subCategory: formData.subCategory,
        image: uploadedImageUrls[0], // First image as main
        images: uploadedImageUrls,
        description: formData.description,
        shortDescription: formData.shortDescription,
        brandId: formData.brandId,
  ...
  const addVariant = () => {
    setFormData({
      ...formData,
      variants: [...formData.variants, { type: '', value: '', sku: '', priceOverride: '', stock: '', image: '' }],
    });
  };
  ...
  const inputClass = "w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 hover:border-gray-400 bg-white";
  const labelClass = "block text-sm font-medium text-gray-700 mb-2";
  const selectClass = "w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 hover:border-gray-400 bg-white cursor-pointer";
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 pb-12">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 mt-1 text-sm">Fill in all required information to list your product</p>
                </div>
              </div>
              <div className="hidden md:flex items-center space-x-2 text-sm">
                <span className="px-3 py-1 bg-primary-100 text-primary-700 rounded-full font-medium">Step 1 of 15</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['identifiers'] = el; }}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">UPC / EAN / ISBN</label>
                <input type="text" name="upc" value={formData.upc} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Internal Product Code</label>
                <input type="text" name="internalCode" value={formData.internalCode} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
              ...
              <div className="flex items-center pt-7">
                <input type="checkbox" name="serialNumberRequired" checked={formData.serialNumberRequired} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Serial Number Required</label>
              </div>
            </div>
          </Section>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" name="costPrice" value={formData.costPrice} onChange={handleChange} min="0" step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
              ...
              <div className="flex items-center pt-7">
                <input type="checkbox" name="taxInclusive" checked={formData.taxInclusive} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Tax Inclusive Price</label>
              </div>
            </div>
          </Section>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" name="minOrderQty" value={formData.minOrderQty} onChange={handleChange} min="1"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Maximum Order Quantity</label>
                <input type="number" name="maxOrderQty" value={formData.maxOrderQty} onChange={handleChange} min="1"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">SKU Status</label>
                <select name="skuStatus" value={formData.skuStatus} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['variants'] = el; }}
          >
                ...
                <div key={index} className="p-4 border border-gray-200 rounded-lg">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <select value={variant.type} onChange={(e) => handleVariantChange(index, 'type', e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
                    ...
                    <div>
                      ...
                      <input type="text" value={variant.value} onChange={(e) => handleVariantChange(index, 'value', e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input type="number" value={variant.priceOverride} onChange={(e) => handleVariantChange(index, 'priceOverride', e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input type="url" value={variant.image} onChange={(e) => handleVariantChange(index, 'image', e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                    </div>
                  </div>
                  <button type="button" onClick={() => removeVariant(index)}
                    className="mt-2 text-red-600 hover:text-red-700 text-sm">Remove Variant</button>
                </div>
              ))}
              <button type="button" onClick={addVariant}
                className="text-primary-600 hover:text-primary-700 text-sm font-medium">+ Add Variant</button>
            </div>
          </Section>
          ...
          >
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Key Features (Min 5)</label>
                {formData.keyFeatures.map((feature, index) => (
                  <input key={index} type="text" value={feature} onChange={(e) => handleFeatureChange(index, e.target.value)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Box Contents</label>
                  <textarea name="boxContents" value={formData.boxContents} onChange={handleChange} rows={2}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Usage Instructions</label>
                  <textarea name="usageInstructions" value={formData.usageInstructions} onChange={handleChange} rows={2}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Care Instructions</label>
                  <textarea name="careInstructions" value={formData.careInstructions} onChange={handleChange} rows={2}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Warranty Details</label>
                  <textarea name="warrantyDetails" value={formData.warrantyDetails} onChange={handleChange} rows={2}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Warranty Duration</label>
                  <input type="text" name="warrantyDuration" value={formData.warrantyDuration} onChange={handleChange}
                    placeholder="e.g., 1 Year"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Return Window (Days)</label>
                  <input type="number" name="returnWindow" value={formData.returnWindow} onChange={handleChange} min="0"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
                </div>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="returnable" checked={formData.returnable} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Returnable Product</label>
              </div>
            </div>
          </Section>
          ...
          >
            <div className="space-y-4">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
                  ...
                  <label
                    ...
                    className={`flex flex-col items-center justify-center cursor-pointer ${
                      imageFiles.length >= 7 ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    ...
                    <p className="text-xs text-gray-500">
                      PNG, JPG, JPEG up to 5MB ({imageFiles.length}/7 uploaded)
                    </p>
                  </label>
                </div>
              ...
              <div>
                ...
                <input type="url" name="productVideo" value={formData.productVideo} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Product Datasheet / Manual (URL)</label>
                <input type="url" name="datasheet" value={formData.datasheet} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
            </div>
          </Section>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" name="netWeight" value={formData.netWeight} onChange={handleChange} min="0" step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Gross Weight (kg)</label>
                <input type="number" name="grossWeight" value={formData.grossWeight} onChange={handleChange} min="0" step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Length (cm)</label>
                <input type="number" name="length" value={formData.length} onChange={handleChange} min="0" step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Width (cm)</label>
                <input type="number" name="width" value={formData.width} onChange={handleChange} min="0" step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Height (cm)</label>
                <input type="number" name="height" value={formData.height} onChange={handleChange} min="0" step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              <div className="flex items-center">
                <input type="checkbox" name="fragile" checked={formData.fragile} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Fragile Item</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="liquid" checked={formData.liquid} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Liquid Item</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="hazardous" checked={formData.hazardous} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Hazardous Item</label>
              </div>
            </div>
          </Section>
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['shipping'] = el; }}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <input type="number" name="processingTime" value={formData.processingTime} onChange={handleChange} min="0"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Shipping Charges</label>
                <select name="shippingCharges" value={formData.shippingCharges} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div className="flex items-center">
                <input type="checkbox" name="codAvailable" checked={formData.codAvailable} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">COD Available</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="internationalShipping" checked={formData.internationalShipping} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">International Shipping</label>
              </div>
            </div>
          </Section>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">FSSAI Number (Food Products)</label>
                <input type="text" name="fssaiNumber" value={formData.fssaiNumber} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Drug License Number (Medical)</label>
                <input type="text" name="drugLicenseNumber" value={formData.drugLicenseNumber} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">BIS / ISI Certification</label>
                <input type="text" name="bisCertification" value={formData.bisCertification} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div className="flex items-center pt-7">
                <input type="checkbox" name="expiryDateRequired" checked={formData.expiryDateRequired} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Expiry Date Required</label>
              </div>
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Safety Disclaimer</label>
                <textarea name="safetyDisclaimer" value={formData.safetyDisclaimer} onChange={handleChange} rows={2}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Legal Disclaimer</label>
                <textarea name="legalDisclaimer" value={formData.legalDisclaimer} onChange={handleChange} rows={2}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
            </div>
          </Section>
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['seo'] = el; }}
          >
            <div className="space-y-4">
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Search Keywords (Backend)</label>
                <input type="text" name="searchKeywords" value={formData.searchKeywords} onChange={handleChange}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['offers'] = el; }}
          >
            <div className="space-y-3">
              <div className="flex items-center">
                <input type="checkbox" name="eligibleForOffers" checked={formData.eligibleForOffers} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Eligible for Platform Offers</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="bankOfferEnabled" checked={formData.bankOfferEnabled} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Bank Offer Enabled</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="flashSaleEligible" checked={formData.flashSaleEligible} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Flash Sale Eligible</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="dealOfDayEligible" checked={formData.dealOfDayEligible} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Deal of the Day Eligible</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="bulkDiscountEnabled" checked={formData.bulkDiscountEnabled} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Bulk Purchase Discount (B2B)</label>
              </div>
            </div>
          </Section>
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['b2b'] = el; }}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Wholesale Price</label>
                <input type="number" name="wholesalePrice" value={formData.wholesalePrice} onChange={handleChange} min="0" step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Minimum Wholesale Quantity</label>
                <input type="number" name="minWholesaleQty" value={formData.minWholesaleQty} onChange={handleChange} min="0"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div className="flex items-center">
                <input type="checkbox" name="tieredPricing" checked={formData.tieredPricing} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Tiered Pricing</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="businessOnlyVisibility" checked={formData.businessOnlyVisibility} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">Business-only Visibility</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" name="gstInvoiceMandatory" checked={formData.gstInvoiceMandatory} onChange={handleChange}
                  className="h-4 w-4 text-primary-600 rounded" />
                <label className="ml-2 text-sm text-gray-700">GST Invoice Mandatory</label>
              </div>
            </div>
          </Section>
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['quality'] = el; }}
          >
            <div className="space-y-4 bg-yellow-50 p-4 rounded-lg border border-yellow-200">
              <div className="flex items-start">
                <input type="checkbox" name="qualityCheckConfirmed" checked={formData.qualityCheckConfirmed} onChange={handleChange}
                  required className="h-4 w-4 text-primary-600 rounded mt-1" />
                <label className="ml-2 text-sm text-gray-700">
                  <span className="font-semibold">* I confirm that I have performed a self quality check</span> on this product and it meets all quality standards.
                </label>
              </div>
              <div className="flex items-start">
                <input type="checkbox" name="authenticityConfirmed" checked={formData.authenticityConfirmed} onChange={handleChange}
                  required className="h-4 w-4 text-primary-600 rounded mt-1" />
                <label className="ml-2 text-sm text-gray-700">
                  <span className="font-semibold">* Product Authenticity Declaration:</span> I declare that this product is genuine and authentic.
                </label>
              </div>
              <div className="flex items-start">
                <input type="checkbox" name="brandAuthorizationConfirmed" checked={formData.brandAuthorizationConfirmed} onChange={handleChange}
                  required className="h-4 w-4 text-primary-600 rounded mt-1" />
                <label className="ml-2 text-sm text-gray-700">
                  <span className="font-semibold">* Brand Authorization:</span> I confirm that I am authorized to sell products under this brand.
                </label>
              </div>
            </div>
          </Section>
          ...
          <Section 
            ...
            onToggle={toggleSection}
            sectionRef={(el) => { sectionRefs.current['status'] = el; }}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Product Visibility</label>
          ...
          {/* Submit Buttons */}
          <div className="sticky bottom-0 mt-8 flex items-center justify-between space-x-4 p-6 bg-white rounded-xl shadow-xl border border-gray-200">
            <div className="text-sm text-gray-600">
              <span className="font-medium">💡 Tip:</span> Save as draft to continue later
            </div>
            <div className="flex items-center space-x-3">
              <Link 
                href="/seller/products" 
                className="px-6 py-2.5 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all duration-200 font-medium hover:border-gray-400"
              >
                Cancel
              </Link>
              <button 
                ...
                className="px-6 py-2.5 border-2 border-amber-400 text-amber-700 bg-amber-50 rounded-lg hover:bg-amber-100 hover:border-amber-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-sm hover:shadow-md transform hover:-translate-y-0.5 disabled:transform-none flex items-center gap-2"
              >
