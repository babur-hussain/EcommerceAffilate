 ...
 */
export const getActiveSponsoredProductIds = async (): Promise<
 ...
 */
export const getRankedProductsByCategory = async (
  category: string
): Promise<IProduct[]> => {
  if (!category) return [];

  const sponsoredProductIds = await getActiveSponsoredProductIds();
  const sponsoredIdStrings = new Set(
    sponsoredProductIds.map((id) => id.toString())
  );

  // Case-insensitive category match using regex
...
export const searchRankedProducts = async (
  ...
  let matchedProducts: Array<IProduct & { score?: number }> =
    textMatches as any;
