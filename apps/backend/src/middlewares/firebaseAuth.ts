...
export const verifyFirebaseToken = async (
  ...
  try {
    ...
    logger.info({ authHeader: authHeader ? authHeader.substring(0, 30) + '...' : 'undefined' }, '🔐 verifyFirebaseToken called');
    ...
    logger.info({ tokenLength: token.length }, '🔍 Attempting to verify Firebase token');
    ...
    // Generic error
    logger.error({ code: error.code, message: error.message, error }, '❌ Firebase token verification error');
